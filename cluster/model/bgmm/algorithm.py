"""
Module bgmm: Bayesian Gaussian Mixture Model
"""
...
class Algorithm:
    ...
    def modelling(self, matrix: np.ndarray, n_components: int, covariance_type: str):
        ...
        try:
            model = sklearn.mixture.BayesianGaussianMixture(
                        ...
                        covariance_prior=None, random_state=self.parameters.random_state, warm_start=False, verbose=0, verbose_interval=10
                    ).fit(X=matrix)
    ...
    def exc(self, matrix: np.ndarray):
        ...
        models = [dask.delayed(self.modelling)(matrix, n_components, covariance_type) 
                  for n_components in self.parameters.array_n_components for covariance_type in self.parameters.array_covariance_type]
